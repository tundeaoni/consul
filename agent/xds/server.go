...
// by checking for `service:write` on the proxied service, or the gateway service.
// This authorization strategy requires that agent/proxycfg only fetches data
// using a token with the same permissions. We assume that any data in the snapshot
// was already filtered, which allows this authorization to only do a single
// authorization check for all the data in a ConfigSnapshot.
